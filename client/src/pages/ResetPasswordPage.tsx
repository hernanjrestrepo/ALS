...
export default function ResetPasswordPage() {
    ...
    async function onSubmit(values: z.infer<typeof formSchema>) {
        ...
        setError(null);
        try {
            await api.post('/auth/reset-password', { email, token, password: values.password });
            setSuccess(true);
            ...
            <div className="flex flex-col min-h-screen items-center justify-center p-4 bg-slate-50 gap-4">
                <Card className="w-full max-w-[400px] shadow-none border-slate-200">
                    <CardHeader className="text-center">
                        <CardTitle className="text-xl font-semibold text-slate-900">Link inválido</CardTitle>
    ...
    return (
        <AuthCard
            wrapperClassName="flex min-h-screen items-center justify-center p-4 bg-slate-50"
            title="Nueva contraseña"
            description={<>Define una nueva contraseña para {email}</>}
        >
            {success ? (
                <div className="flex flex-col items-center gap-3 py-4 text-center">
                    <CheckCircle2 className="h-10 w-10 text-emerald-600" />
                    <p className="text-sm text-slate-700">Contraseña actualizada. Redirigiendo a inicio de sesión...</p>
                </div>
            ) : (
                <Form {...form}>
                    <form onSubmit={form.handleSubmit(onSubmit)} className="space-y-4">
                        ...
                        <AuthSubmitButton isLoading={isLoading} label="Guardar nueva contraseña" />
                    </form>
                </Form>
            )}
        </AuthCard>
