...
export class TemplateDataMapper {
    ...
    public generateData(): Record<string, any> {
        ...
        data['resultados_laboratorio'] = (data['resultados_laboratorio'] || []).map((r: any) => ({
            ...r,
            parametro_nombre: r.parametro,
            parametro_unidad: r.unidad,
            resultado_valor: r.punto_1 || ''
        }));
        data['tiene_resultados_laboratorio'] = data['resultados_laboratorio'].length > 0;

        const infiltracionResults = rawResultadosForLabs.filter((r: any) => (r.parametro || '').toLowerCase().includes('infiltra'));
        data['pruebas_infiltracion'] = infiltracionResults.map((r: any) => ({ infiltracion_parametro: r.parametro, infiltracion_valor: r.valor }));
        ...
        data['puntos_monitoreo'] = data['puntos_monitoreo'].map((p: any) => ({
            ...p,
            punto_descripcion: p.nombre,
            punto_hora: p.hora,
            punto_cota: 'N.A.',
            punto_latitud_gms: p.latitud,
            punto_longitud_gms: p.longitud,
            punto_norte_or: p.norte,
            punto_este_or: p.este
        }));
