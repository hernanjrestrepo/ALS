        ...
        if body is not None:
            ...
            for p in body.findall('.//w:p', namespaces):
                ...
                if 'Nota:' in full_text or 'NOTA:' in full_text:
                    to_remove.append(p)
            for p in to_remove:
                # Find parent of p to remove it safely
                # In ElementTree, we don't have getparent() so we have to search
...
def process_docx(file_path):
    ...
    if os.path.exists(backup_path):
        with open(backup_path, 'rb') as f_in, open(file_path, 'wb') as f_out:
    ...
    # Read from the fresh file (which is identical to backup now)
    with zipfile.ZipFile(backup_path, 'r') as zin, zipfile.ZipFile(file_path, 'w') as zout:
        for item in zin.infolist():
            content = zin.read(item.filename)
            # ONLY modify document.xml and headers/footers
